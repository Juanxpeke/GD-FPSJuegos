...
func show_skills() -> void:
	...
	for i in range(skills_options_container.get_child_count()):
		if i < skill_id_pool.size():
			...
			skills_options_container.get_child(i).hide() # REVIEW: In case the pool grows this is a bug
...
func select_random_skill() -> void:
	MultiplayerManager.log_msg("force close")
	var random_index = rng.randi_range(0, get_child_count() - 1) 
	skills_options_container.get_child(random_index).select()
